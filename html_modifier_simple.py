...
class HTMLModifier:
    ...
    def replace_nav_menu(self, html_content, current_unit, unit_themes, current_path):
        """Reemplaza el nav__menu con el nuevo menú de unidades"""
        if not unit_themes:
            return html_content

        m = self.RE_NAV_MENU.search(html_content)
        if m:
            opening, _, closing = m.groups()
            new_menu = self.generate_units_menu(current_unit, unit_themes, current_path)
            html_content = html_content.replace(m.group(0), opening + new_menu + closing)
        return html_content
        ...
        try:
            ...
            if unit_themes:
                html_content = self.replace_nav_menu(
                    html_content, current_unit, unit_themes, file_path
                )
