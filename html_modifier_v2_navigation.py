...
class HTMLModifier:
    ...
    def normalize_override_relative_path(self, input_path, shared_unit=False):
        ...
        for prefix in (self.output_dir, self.base_dir):
        ...
        if parts[0] in ("asignaturas-muestra", "asignaturas-produccion", "out", "overrides"):
            parts = parts[1:]

        if len(parts) < 2:
            raise ValueError("no se pudo identificar asignatura y unidad")
        ...
        if remainder and remainder[0] == parts[unit_index]:
            remainder = remainder[1:]
    ...
    def print_overrides(self):
        """Imprime overrides disponibles."""
        if not self.overrides_dir.exists():
            print(f"No existe directorio de overrides: {self.overrides_dir}")
            return 0
    ...
    def validate_overrides(self):
        """Valida que los overrides apunten a asignaturas conocidas."""
        if not self.overrides_dir.exists():
            print(f"No existe directorio de overrides: {self.overrides_dir}")
            return True
        ...
        if is_valid:
            print("Overrides validados correctamente")
        else:
    ...
    def apply_overrides_for_subject(self, subject):
        ...
        subject_base_name = Path(subject).name
        subject_override_dir = self.overrides_dir / subject_base_name
        if not subject_override_dir.exists():
            return 0

        applied_count = 0
        print(f"  🎨 Aplicando overrides para {subject_base_name}...")

        for override_file in self.iter_override_files(subject):
            relative_path = override_file.relative_to(self.overrides_dir)
            target_file = self.output_dir / relative_path

            if not target_file.exists():
                print(f"  ⚠ Override ignorado, no existe target: {relative_path}")
                continue

            shutil.copy2(override_file, target_file)
            print(f"  ✓ Override aplicado: {relative_path}")
            applied_count += 1
    ...
    def generate_new_header_bars(self, current_unit, unit_themes, current_path, subject_name):
        ...
        subject_names = {
            ...
            'derecho-1': 'Derecho I',
            'cibernetica-1': 'Cibernética y Computación I',
            ...
            'biologia-1': 'Biología I',
            'biologia-2': 'Biología II',
            'biologia-3': 'Biología III',
            'biologia-4': 'Biología IV',
            'calculo-1': 'Cálculo Diferencial e Integral I',
            'calculo-2': 'Cálculo Diferencial e Integral II',
            'politicas-1': 'Ciencias Políticas y Sociales I',
            'politicas-2': 'Ciencias Políticas y Sociales II',
            'salud-2': 'Ciencias de la Salud II',
            'economia-1': 'Economía I',
            'economia-2': 'Economía II',
            'estadistica-2': 'Estadística y probabilidad II',
            'filosofia-1': 'Filosofía I',
            'filosofia-2': 'Filosofía II',
            'geografia-1': 'Geografía I',
            'geografia-2': 'Geografía II',
            'historia-mexico-1': 'Historia de México I',
            'historia-universal-2': 'Historia Universal, Moderna y Contemporanea II',
            'latin-1': 'Latín I',
            'latin-2': 'Latín II',
            'matematicas-4': 'Matemáticas IV',
            'psicologia-2': 'Psicología II',
            'quimica-4': 'Química IV',
            'comunicacion-1': 'Taller de Comunicación I',
            'comunicacion-2': 'Taller de Comunicación II',
            'taller-diseno-1': 'Taller de Diseño Ambiental I',
            'taller-diseno-2': 'Taller de Diseño Ambiental II',
            'taller-grafica-1': 'Taller de Expresión Gráfica I',
            'taller-grafica-2': 'Taller de Expresión Gráfica II',
            'taller-lectura-1': 'Taller de lectura y análisis de textos literarios I',
            'taller-lectura-2': 'Taller de lectura y análisis de textos literarios II',
            'temas-filosofia-1': 'Temas Selectos de Filosofía I',
            'teoria-historia-1': 'Teoría de la Historia I',
            'tlriid-3': 'Taller de Lectura, Redacción e Iniciación a la Investigación Documental III',
        }
...
'''
    ...
    def process_subject(self, subject):
        ...
        overrides_applied = self.apply_overrides_for_subject(subject)
        if overrides_applied:
            print(f"  ✅ Overrides aplicados: {overrides_applied}")
...
def main():
    ...
    if args.list_overrides:
        modifier.print_overrides()
        return

    if args.validate_overrides:
        if not modifier.validate_overrides():
            sys.exit(1)
        return
